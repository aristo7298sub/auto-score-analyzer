...
class AnalysisService:
    @staticmethod
    async def analyze_score(score: StudentScore) -> ScoreAnalysis:
        """分析学生成绩"""
        try:
            # 初始化Azure OpenAI客户端
...
"""
    ...
    @staticmethod
    async def analyze_scores_batch(scores: List[StudentScore], max_concurrent: int = 50) -> List[StudentScore]:
        """
        批量分析学生成绩，支持并发处理
        ...
        async def analyze_with_semaphore(score: StudentScore) -> StudentScore:
            """使用信号量控制的分析函数"""
            async with semaphore:
                try:
                    analysis = await AnalysisService.analyze_score(score)
                    # 将分析结果添加到原始score对象中
        ...
        analyzed_scores = await asyncio.gather(*tasks, return_exceptions=True)
        
        # 过滤掉可能的异常结果
        return [s for s in analyzed_scores if isinstance(s, StudentScore)] 